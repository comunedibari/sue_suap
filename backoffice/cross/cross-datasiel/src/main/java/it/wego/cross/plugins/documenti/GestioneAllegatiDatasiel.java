...
import it.wego.cross.action.ProtocolloDatasielAction;
 ...
 * @author CS
 */
public class GestioneAllegatiDatasiel implements GestioneAllegati {

    @Autowired
    private ProtocolloDatasielAction protocolloAction;
...
//        }
    ...
    public byte[] getFileContent(Allegati allegato, Enti ente, LkComuni comune) throws Exception {
        if (!Utils.e(allegato.getIdFileEsterno())) {
            return protocolloAction.getAllegato(allegato.getIdFileEsterno());
    ...
    public Allegato getFile(String idFileEsterno, Enti ente, LkComuni comune) throws Exception {
        ...
        byte[] content = protocolloAction.getAllegato(String.valueOf(idFile));
