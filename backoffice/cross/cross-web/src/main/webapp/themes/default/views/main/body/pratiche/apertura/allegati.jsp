...
<div class="inlineLabels">
    <table cellspacing="0" cellpadding="0" class="master">
        ...
        <c:if test="${not empty pratica.allegati}">
			...
			<c:if test="${allegato.descrizioneEvento == ('(proc) Invio Comunicazione / Integrazione')}">
                ...
                <tr>
                	<td style="background-color: <%= colorIntegrazione%>"><span id="allegato_${allegato.id}">${allegato.descrizioneEvento}</span></td>
                    ...
                    <td style="background-color: <%= colorIntegrazione%>">${allegato.tipoFile}</td>
                    <td style="background-color: <%= colorIntegrazione%>">
                        <div style="text-align: center;">
                            <a class="scarica2" href="${path}/download.htm?id_file=${allegato.id}" target="_blank">.</a>
                        </div>
                    </td> 
                    <c:if test="${isAllegatoModificabile=='TRUE'}">
                        <td style="background-color: <%= colorIntegrazione%>">
                            <div style="text-align: center;">
                                <a class="icona_modifica" href="#" onclick="modificaDescrizione('${allegato.id}', '${allegato.descrizione}')"> </a>
                            </div>
                        </td>
                    </c:if>
                </tr>
