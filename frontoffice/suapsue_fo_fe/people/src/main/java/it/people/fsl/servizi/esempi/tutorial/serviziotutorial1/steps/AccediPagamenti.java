...
public class AccediPagamenti extends Step implements IStartPayment {
	private static final boolean TEST_EXTENDED_MIP = false;
	private static final boolean TEST_OBSERVER_DATA = true;
    ...
    public PaymentRequestParameter getPaymentRequestParameter() {
		 ...
		 */
        try {
            param.getUserData().setEmailUtente(new InternetAddress(this.email));
		} catch(Exception ex) {}
		...
		// Informazioni passate direttamente all'Observer
		if (TEST_OBSERVER_DATA)
		    param.getObserverData().setData("<richiesta><testo>Informazioni di Prova passate all'observer</testo><idOperazione></idOperazione></richiesta>");
		
		if (TEST_EXTENDED_MIP) {
		    // i seguenti parametri sono supportati solo dalla
		    // nuova versione del MIP
		    param.getServiceData().setAnnoDocumento("2005");
		    param.getServiceData().setNumeroDocumento("ABCD1234");
		    
		    // Importi contabili
		    ArrayList importiContabili = param.getAccountingData().getImportiContabili();
		    importiContabili.add(new ImportoContabile("IC1", new Long(1000)));
		    importiContabili.add(new ImportoContabile("IC2", new Long(2000)));		    
		    
		    // EntiDestinatari
		    ArrayList entiDestinatari = param.getAccountingData().getEntiDestinatari();
		    
		    entiDestinatari.add(new EnteDestinatario(
		            "IDCAUSALE1", 
		            new Long(1100), 
		            "CAUSALE1", 
		            "IDIMPORTOCONTABILEINGRESSO1", 
		            "IDIMPORTOCONTABILEUSCITA1"));
		    
		    entiDestinatari.add(new EnteDestinatario(
		            "IDCAUSALE2", 
		            new Long(2200), 
		            "CAUSALE2", 
		            "IDIMPORTOCONTABILEINGRESSO2", 
		            "IDIMPORTOCONTABILEUSCITA2"));
		}		
    ...
    public boolean logicalValidate(AbstractPplProcess process,
        ...
        process.debug("Invocata la logical validate dello step di accesso ai pagamenti");
        return true;
    }
