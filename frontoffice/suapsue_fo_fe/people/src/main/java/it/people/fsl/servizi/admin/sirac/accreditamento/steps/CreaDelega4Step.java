 ...
 */
public class CreaDelega4Step extends Step {
  ...
  public void service(AbstractPplProcess process, IRequestWrapper request) throws IOException, ServletException {
    ...
    IAccreditamentoClientAdapter accr = null;
    ...
    try {
	    ...
	    accr = new IAccreditamentoClientAdapter(IAccreditamentoURLString);
	    ProfiloLocale profilo = UtilHelper.getProfiloLocale(myData, accr, codiceFiscaleUtente, idComune);
		if (profilo == null) {
			rwh.addSiracError("CreaDelega4Step::Errore1", "Non esiste un profilo locale per l'utente " + codiceFiscaleUtente);
		    updateWorkflow(process);
		    return;
	    }
      ...
      rwh.addSiracError("CreaDelega4Step::Service()::Eccezione1", ex.getMessage());
    } finally {
      updateWorkflow(process);
      request.setAttribute("delegaForm", delegaForm);
    }
