 ...
 */
public class CreaDelega1Step extends Step {

  private static Logger logger = LoggerFactory.getLogger(CreaDelega1Step.class.getName());
  ...
  public void service(AbstractPplProcess process, IRequestWrapper request) throws IOException, ServletException {
	  ...
	  try {
	      ...
	      if (profilo == null) {
	        rwh.addSiracError("CreaDelega1Step::Errore1", "Non esiste un profilo locale per l'utente " + codiceFiscaleUtente);
	        updateWorkflow(process);
	        return;
	      }
	      
	      Accreditamento currAccr = (Accreditamento)session.getAttribute(SiracConstants.SIRAC_ACCR_ACCRSEL);
	      if(ProfiliHelper.TIPOQUALIFICA_INTERMEDIARIO.equals(currAccr.getQualifica().getTipoQualifica()) && "Rappresentante CAF".equals(currAccr.getQualifica().getDescrizione())){
	    	  session.setAttribute("accreditamentoCAF", currAccr);
	      }
	      ...
	      if (logger.isDebugEnabled()) {
	        logger.debug("CreaDelega1Step::Service():: Recuperati accreditamenti di " 
	      ...
	      Accreditamento[] accrCAF = filterAccreditamentiByDescription(accrs, "Rappresentante CAF");
	      
	      // Memorizza l'elenco degli accreditamenti nel ProcessData
	      myData.setElencoAccreditamenti(accrCAF);
	      
	      request.setAttribute("accreditamenti", accrCAF);
	      
	//      // Verifica se con l'accreditmento corrente memorizzato in sessione � possibile creare delegati
	...
	//      
	//      String tipoQualificaAccr = null;
	//      String descrQualificaAccr = null;
	//      if (curAccr != null) {
	//        tipoQualificaAccr = curAccr.getQualifica().getTipoQualifica();
	//        descrQualificaAccr = curAccr.getQualifica().getDescrizione();
	//      }
	//      
	//      
	//      if (!ProfiliHelper.TIPOQUALIFICA_INTERMEDIARIO.equals(tipoQualificaAccr) 
	//          || !"Rappresentante CAF".equals(descrQualificaAccr)) {
	//        rwh.addSiracError("CreaDelega1Step::Service()::Errore", "L'accreditamento corrente non � abilitato al servizio selezionato.");
	//      }
    
    } catch (Exception ex) {
      rwh.addSiracError("CreaDelega1Step::Service()::Errore", ex.getMessage());
    } finally {
      updateWorkflow(process);
    }
  }
  ...
  private Accreditamento[] filterAccreditamentiByDescription(Accreditamento[] accrs, String descrizioneQualifica){
	  ...
	  for(int i=0; i < accrs.length; i++){
		  if(ProfiliHelper.TIPOQUALIFICA_INTERMEDIARIO.equals(accrs[i].getQualifica().getTipoQualifica()) && "Rappresentante CAF".equals(accrs[i].getQualifica().getDescrizione())){
