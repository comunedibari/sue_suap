 ...
 */
public class CreaDelega2Step extends Step {

  private static Logger logger = LoggerFactory.getLogger(CreaDelega2Step.class
      .getName());
  ...
  public void service(AbstractPplProcess process, IRequestWrapper request)
    ...
    try {
	    ...
	    if(myData.getSelezioneAccreditamentoIndex() == 999){
	    	accrSelezionato = (Accreditamento)session.getAttribute("accreditamentoCAF");
	    } else {
	    ...
	    if (profilo == null) {
	    	rwh.addSiracError("CreaDelega2Step::Errore1", "Non esiste un profilo locale per l'utente " + codiceFiscaleUtente);
	    ...
	    profiloOperatore.setCodiceFiscale(authUserData.getCodiceFiscale());
      	...
      	Qualifica qualificaOAI = accr.getQualificaById("OAI");
      	if (logger.isDebugEnabled()) {
      		logger.debug("CreaDelega2Step::Service():: Recuperata descrizione qualifica OAI:"
      				+ qualificaOAI);
      	}
      
      	delegaForm.setIdQualifica("OAI");
      	delegaForm.setDescrQualifica(qualificaOAI.getDescrizione());
    
    } catch (Exception ex) {
      rwh.addSiracError("CreaDelega2Step::Service()::Eccezione1", ex.getMessage());
    } finally {
      updateWorkflow(process);
    }
  }
