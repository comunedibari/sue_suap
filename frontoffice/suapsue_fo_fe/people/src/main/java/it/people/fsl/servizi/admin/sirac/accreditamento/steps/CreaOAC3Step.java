 ...
 */
public class CreaOAC3Step extends Step {
  
  private static Logger logger = LoggerFactory.getLogger(CreaOAC3Step.class.getName());
  ...
  public void service(AbstractPplProcess process, IRequestWrapper request) throws IOException, ServletException {
    ...
    try {
		...
		if (profilo == null) {
			rwh.addSiracError("CreaOAC3Step::Errore1", "Non esiste un profilo locale per l'utente " + codiceFiscaleUtente);
	    ...
	    if (logger.isDebugEnabled()) {
	      logger.debug("CreaOAC3Step::Service():: Recuperato template delega per idqualifica=" + delegaForm.getIdQualifica()+": " 
	          + delegaTemplate);
	    }
      ...
      rwh.addSiracError("CreaOAC3Step::Service()::Eccezione1", ex.getMessage());
    } finally {
      updateWorkflow(process);
    }
  }
...
//	    byte[] bb =  Base64.decodeBase64(delegaForm.getCertificazione().getBytes());
//	    PKCS7Parser p7mParser = new PKCS7Parser(bb);
