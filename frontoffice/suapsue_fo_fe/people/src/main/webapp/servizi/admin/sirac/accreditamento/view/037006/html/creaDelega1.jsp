...
<%
  ...
  Accreditamento accrCAF = (Accreditamento)session.getAttribute("accreditamentoCAF");
  ProcessData myData = (ProcessData) pplProcess.getData();
	
%>

  <div class="admin_sirac_accr_titolo">
      <bean:message key="label.creaDelega1.titolo" />
  </div>
...
%>
	  <div class="text_block" align="left">
	    <h2>
  		  <bean:message key="label.creaDelega1.alreadyCAF1" /><br>
   		  <bean:message key="label.creaDelega1.alreadyCAF2" />
        </h2>
	    ...
	    <table class="table_data" width="100%">
          ...
          <tbody>
			  ...
			  <td><%=accrCAF.getQualifica().getDescrizione()%></td>            
              <td><%=accrCAF.getProfilo().getCodiceFiscaleIntermediario()%></td>            
              <td><%=accrCAF.getProfilo().getPartitaIvaIntermediario() %></td>            
              <td><%=accrCAF.getProfilo().getDenominazione() %></td>
			</tr>
          </tbody>
	 	</table>
	  </div>
	  ...
	  <div class="text_block" align="left">
	    <h2>
	      <bean:message key="label.creaDelega1.info" />
	    </h2>
	  </div>
<%
	} 
		    	...
		    	<td>
				    <c:choose>
				      <c:when test="${empty accreditamenti}">
						<bean:message key="error.creaDelega.nessunAccreditamentoCAF1" /><br>
						<bean:message key="error.creaDelega.nessunAccreditamentoCAF2" />
				      </c:when>
				      <c:otherwise>
				          <table class="table_data" width="100%">
					          <thead>
