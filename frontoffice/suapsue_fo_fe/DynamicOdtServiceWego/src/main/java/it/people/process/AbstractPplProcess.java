 ...
 * <br>
 *
 * Questa classe e' il padre di tutti i processi e ne definisce le caratteristiche
 * principali. Tutte le classi che rappresentano uno specifico processo devono
...
public abstract class AbstractPplProcess extends DynaActionForm {
    ...
    protected static final String ANONYMOUS_ID_PREFIX = PeopleConstants.ANONYMOUS_USERID.substring(0, 16);
    ...
    // Logger
    private static Logger logger = LoggerFactory.getLogger(AbstractPplProcess.class);
    ...
    /**
     * Errori di validazione che devono essere mostrati all'interno 
     * del servizio, sono gestiti attraverso il framework Struts e 
     * mostrati attraverso il custom tag PeopleErrorsTag.
     */
    protected IValidationErrors errors = null;
    /**
     * Errori generali del servizio e del framework che devono essere
     * mostrati all'interno dello step del servizio sempre attraverso
     * il custom tag PeopleErrorsTag.
     * L'array contiene le chiavi dei messaggi di errore che dovranno
     * essere specificati in uno dei file di properties previsti. 
     */
    protected ArrayList serviceErrors = new ArrayList();

    /**
     * Il metodo permette di aggiungere un messaggio di errore che deve
     * essere mostrato all'interno del servizio attraverso il custom tag 
     * PeopleErrorsTag, il messaggio sar? accodato agli eventuali messaggi
     * di errore riportati dalla validazione. N.B. il messaggio non ? 
     * la chiave ma il testo vero e proprio.
     * @param message
     */
    ...
    /**
     * Il metodo ritorna l'elenco delle chiavi dei messaggi di errore generici. Per generici
     * si intendono tutti i messaggi di errore che devono essere mostrati
     * all'interno dello step del servizio ma che non sono collegati alla validazione.   
     * @return
     */
     ...
     * getNewIdentificativoOperazione() ma non ? ancora stata effettuata
     * una chiamata al web-service di back-end attraverso callService(),
     * allora l'id ritornato ? il nuovo id generato altrimenti l'id ritornato
     * ? l'identificativo della pratica.
     * N.B. L'identificativo generato ? sempre rimosso dall'invocazione 
     * della callService(), anche in caso di errore, ? quindi sempre
     * necessario rigenerarlo prima di effettuare nuove chiamate al back-end.    
     * 
    ...
    public String getCurrentOperationId() {
        ...
        if (currentOperationId == null) {
            IdentificatoreUnivoco idUnivoco = ((AbstractData) this.getData()).getIdentificatoreUnivoco();
            return idUnivoco.getCodiceSistema().getCodiceIdentificativoOperazione();
        } else {
    ...
    public void init() {
         ...
         * 
         * La action form di un nuovo procedimento contiene ancora la lista degli
         * errori accumulati in un procedimento precedente, ? necessario azzerarli
         */
        this.errors = null;
    ...
    /**
     * istanzio il DTO corretto rispetto allo STEP corrente
     *  sulla base delle informazioni presenti nel process
     */
    ...
    public void writeDto() throws MappingException {
        if (getView().getCurrentActivity().getCurrentStep() instanceof DtoStep) {
            ((DtoStep) getView().getCurrentActivity().getCurrentStep()).writeDto(getData(), (PeopleDto) get("dto"));
        }
    }
    ...
    public void readDto() throws MappingException {
        if (getView().getCurrentActivity().getCurrentStep() instanceof DtoStep) {
            ((DtoStep) getView().getCurrentActivity().getCurrentStep()).readDto(getData(), (PeopleDto) get("dto"));
        }
    }
    ...
    public String getParameter(String paramName)
        ...
        return ParameterHelper.getInstance(this.getCommune()).getParameter(this, paramName);
    }

    /**
     * @see it.people.util.params.IParamHelper#paramExist(it.people.process.AbstractPplProcess, java.lang.String)
     */
    public boolean paramExist(String paramName) {
        return ParameterHelper.getInstance(this.getCommune()).paramExist(this, paramName);
    }

    /**
     * ritorna la Collection con i dati della TABELLA di DOMINIO
     * @param tableID ID tabella di dominio
     * @return
     * @throws TableNotFoundException
     */
    public Collection getTableOptions(String tableID) throws TableNotFoundException {
        if (getCommune() != null) {
            return TableHelper.getInstance(this.getCommune()).getTableValues(getProcessName(), getCommune().getOid(), tableID);
        } else {
            return null;
        }
    }
    ...
    /**
     * ritorna la Collection con i dati della TABELLA di DOMINIO
     * per la quale nella colonna indicizzata da indexFilterColumn c'e' il valore valueFilterColumn
     * @param tableID ID tabella di dominio
     * @return
     * @throws TableNotFoundException
     */
    public Collection getTableOptionsFiltered(String tableID, int indexFilterColumn, String valueFilterColumn) throws TableNotFoundException {
        if (getCommune() != null) {
            Collection allTableValues = TableHelper.getInstance(this.getCommune()).getTableValues(getProcessName(), getCommune().getOid(), tableID);
            ...
            while (it.hasNext()) {
                ...
                valueOfIndexedColumnInCurrentRow = ob.getLabel(indexFilterColumn - 1);
                if (valueOfIndexedColumnInCurrentRow.equalsIgnoreCase(valueFilterColumn)) {
    ...
    /**
     * Ottiene dal workflow la classa da istanziare
     * per visualizzare il processo
     * @param comune
     * @param device
     * @throws Exception
     * @throws peopleException
     */
    protected void createView(City comune, Device device)
            throws Exception, peopleException {
        ...
        try {
            ...
            logger.error("", e);
            throw new Exception("Cannot create view. Unable to instantiate class " + processView +
    ...
    /**
     * Ottiene dal workflow la classe da istanziare
     * per stampare il processo
     * @param comune
     * @throws Exception
     * @throws peopleException
     */
    protected void createPrint(City comune)
            throws Exception, peopleException {
    ...
    protected void createModel(City comune, Device device, PeopleContext context)
        ...
        if (m_data instanceof AbstractData) {
             ...
             *                            riguardo a Richiedente e Titolare in modo
             *                            che possano essere recuperate al ripristino
             *                            della pratica
             */
            ...
            if (theData.getIdentificatorePeople() == null) {
                ...
                if (comune.getAooPrefix() == null || "".equals(comune.getAooPrefix())) {
                    throw new Exception("Impossibile inizializzare la pratica il prefisso di Aoo non pu? essere nullo");
                }
                ...
                IdentificatoreUnivoco idUn = new IdentificatoreUnivoco(id, context.getCommune().getOid());
    ...
    protected String generaIdentificativoOperazione(City commune, PeopleContext context) {
        // il sirac ritorna il codice fiscale anche per l'utente anonimo
        String codiceFiscale = context.getUser().getUserData().getCodiceFiscale();
        return IDGenerator.generateID(codiceFiscale, commune.getAooPrefix());
    // return IDGenerator.generateID(userPrefix + "-" + commune.getAooPrefix());
    }

    /**
     * Ritorna un nuovo identificativo, ? da utilizzare nella costruzione dell'xml
     * inviato ai servizi di back-office di supporto.
     * Prestare attenzione al fatto che all'invocazione del metodo l'id generato
     * ? salvato temporaneamente per essere utilizzato alla prima chiamata ad un servizio
     * di back-end attraverso la callService(). Non appena effettuata la chiamata con 
     * successo l'id temporaneo ? consumato ed ? necessario generarne uno nuovo per le
     * successive chiamate.
     * Per la chiamata ai back-end finali per i servizi di visura non dovrebbe essere
     * generato un nuovo identificativo, in questo modo l'id utilizzato ? quello della 
     * pratica. 
     * @return un nuovo identificativo operazione. 
     */
    public String getNewIdentificativoOperazione() {
        this.currentOperationId = generaIdentificativoOperazione(this.getCommune(), this.m_context);
        return this.currentOperationId; //generaIdentificativoOperazione(this.getCommune(), this.m_context);

    }

    public static boolean isAnonymousPplProcess(String identificatoreProcedimento) {
        return identificatoreProcedimento.startsWith(ANONYMOUS_ID_PREFIX);
    }

    /**
     * Valida i dati, restituisce un 'ActionErrors' contenete gli eventuali errori.
     * @param mapping
     * @param request
     * @return
     */
    public ActionErrors validate(
        ...
        // E' necessario distinguere tra la validazione relativa all'upload di un 
        // file e la validazione vera e propria e relativa alla verifica dei dati 
        // inseriti.

        if (SAVE_UPLOAD_ACTION_PATH.equals(mapping.getPath())) {
            logger.debug("Validazione allegati caricati.");

            // Verifica che l'eventuale allegato non superi la dimensione massima consentita
            // In caso di errore esce dalla validazione senza richiamare quella degli step
            Boolean maxLengthExceeded = (Boolean) request.getAttribute(
                    MultipartRequestHandler.ATTRIBUTE_MAX_LENGTH_EXCEEDED);

            if ((maxLengthExceeded != null) && (maxLengthExceeded.booleanValue())) {
                this.errors.add(MESSAGE_KEY_MAX_LENGTH_EXCEEDED, new ActionError(MESSAGE_KEY_MAX_LENGTH_EXCEEDED));
                logger.info("Inserito allegato di dimensioni superiori a quelle previste");
                return (ActionErrors) this.errors;
            }

            // Verifica che la dimensione complessiva degli allegati non superi il limite
            ...
            if (this.m_data instanceof AbstractData) {
                ...
                if (uploadedFileSize > 0) {
                    ...
                    int totalLength = uploadedFileSize; // dimensione dell'ultimo allegato caricato
                    ...
                    int maxLength = 0;
                    try {
                        maxLength = Integer.parseInt(PeopleProperties.ATTACHMENT_MAX_TOTAL_SIZE.getValueString(this.m_commune.getOid())) * 1024;
                    } catch (Exception ex) {
                    ...
                    // N.B. L'inserimento dell'allegato avviene solo se ? superata la validazione
                    if (maxLength > 0 && totalLength > maxLength) {
                        logger.info("La dimensione massima degli allegati inseriti supera il limite previsto");
                        this.errors.add(MESSAGE_KEY_MAX_TOTAL_LENGTH_EXCEEDED, new ActionError(MESSAGE_KEY_MAX_TOTAL_LENGTH_EXCEEDED));
                    }
                }
            }
            ...
            getView().propertyUpdate(RequestUtils.getChangedProperties(this, request, "data"));

            // Validazione dei contenuti
            try {
                getView().getCurrentActivity().validate(
                        this,
                        getServlet().getServletContext(),
                        request,
                        errors, mapping.getPath());
            } catch (ParserException pEx) {
            	logger.error("", pEx);
                this.errors.add(pEx.getPropertyName(), new ActionError(pEx.getPropertyName()));
            }
        }
    ...
    public String getIdentificatoreProcedimento() {
        return ((AbstractData) this.getData()).getIdentificatorePeople().getIdentificatoreProcedimento();
    }
    ...
    public PplPrincipal[] findPrincipalWithDelegate(PplUser user) {
        ...
        if (user != null) {
            ...
            PplDelegate delegate =
                    (PplDelegate) m_delegates.get(user.getUserID());
            if (delegate != null) {
                iter = m_principals.keySet().iterator();
                while (iter.hasNext()) {
                    PplPrincipal princ = (PplPrincipal) iter.next();
                    if (delegate.getOwner().getUserID().equals(princ.getUserID())) {
                        result.add(
                                new PplPrincipal(
                                delegate.getUserID(),
                                princ.getRole()));
     ...
     * @deprecated Il metodo non va utilizzato, utilizzare al suo posto la callService che accetta l'xml sottoforma di stringa
     * @param serviceName
     * @param inParameter
     * @return
    ...
    public String callService(String serviceName, XmlObject parameter)
        ...
        return this.callService(serviceName, XmlObjectWrapper.generateXml(parameter));
    }
    ...
    public String callService(String serviceName, String parameter)
            throws ServiceException, SendException {
        IDataAccess da = DAOFactory.getInstance().create(serviceName, getCommune().getOid());

        try {
            // intercetto il tracciato per il back-end e lo imbusto

            IRequestEnvelope envelopeBean = EnvelopeHelper.createEnvelopeFromPplProcessAndSession(this, getContext().getSession());
            ...
            String xmlResponse = da.call(envelopeXML,
                    getCommune().getOid(),
            ...
            // Rimuove dal messaggio di errore l'intestazione 'java.rmi.RemoteException: '
            //String message = sendEx.getMessage().replaceFirst("java.rmi.RemoteException: ", "");
    ...
    /**
     * Mostra il messaggio di errore di collegamento con il web-service di back-end 
     * nello step solo se previsto dalla configurazione del servizio stesso.
     * L'errore specifico ? registrato nel log.
     * @param message
     */
    protected void showBackEndError(String beServiceName, Throwable ex) {
        logger.error("Errore nel collegamento al back-end." + " Servizio di front-end: " + this.getProcessName() + "." + " Servizio di back-end: " + beServiceName);

        if (this.getView().isShowBackEndError()) {
            Object args[] = {};
            String message = MessageBundleHelper.message(
                    "errors.CallServiceError",
                    args,
                    this.getProcessName(),
                    this.getCommune().getKey(),
    ...
    public void setReceiptMailAttachment(boolean receiptMailAttachment) {
        ((AbstractData) this.m_data).setReceiptMailAttachment(receiptMailAttachment);
    }
    ...
    /**
     * Permette a runtime di abilitare e disabilitare la firma 
     * del riepilogo, solo se questa ? stata impostata come abilitata
     * a livello di workflow.xml.
     * Se la firma non ? stata abiltata a livello di workflow, l'impostazione
     * ? ininfluente.
     */
     ...
     * @param state stato del riepilogo
     */
    public void setSummaryState(SummaryState state) {
        ...
        if (summaryActivity != null) {
            ...
            summaryActivity = SummaryActivity.create(getSign() != null, state, view);
            if (summaryActivity != null) {
    ...
    protected it.people.logger.Logger getLogger() {
        return it.people.logger.Logger.getLogger(
                this.getProcessName(),
                this.getCommune());
    }
    ...
    protected int getUploadedFileSize() {
        try {
            FormFile ff = (FormFile) PropertyUtils.getProperty(this.getData(), SaveUploadFile.PARAMETER_UPLOADFILE);
            if (ff != null) {
