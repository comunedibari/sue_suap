...
//   FIX 2007-06-13 - Aggiunto domicilio elettronico
   ...
   public static final String[] ACCEPTED_DATE_PATTERNS = {"dd/MM/yyyy", "dd-MM-yyyy", "ddMMyyyy", "yyyy-MM-dd", "yyyy/MM/dd", "yyyyMMdd", "EEE MMM dd HH:mm:ss z yyyy"};
  ...
  /*
  public void initProfilo(ProfiloPersonaFisica initProfile) {
    nome = initProfile.getNome();
    cognome = initProfile.getCognome();
    codiceFiscale = initProfile.getCodiceFiscale();
    sesso = initProfile.getSesso();
    dataNascita = initProfile.getDataNascita();
    luogoNascita = initProfile.getLuogoNascita();
    provinciaNascita= initProfile.getProvinciaNascita();
    indirizzoResidenza= initProfile.getIndirizzoResidenza();
  ...
  */
  ...
  private Date parseDateString (String dateString) throws Exception {
  	...
  	if(tempDate == null){
  		throw new Exception("Unable to find suitable date pattern for the date string: " + dateString);
  	}
  	
  	return tempDate;
  }
  
  private Date parseDateString (String dateString, String datePattern) throws Exception {
      ...
      } else throw new Exception("Invalid Date:" + dateString);
      return parsedDate;
    }
