...
var table_name = 'settori_attivita';
var sort_field = 'cod_sett';
...
{name: 'cod_sett'},
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
             {name: 'des_sett_<%=lingueTotali.get(i)%>'},
             <%}%>
{name: 'des_sett_it'}
];

var dyn_fields_protection = ['cod_sett'];
...
labelStyle: 'font-weight:bold;'
            ...
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
            {
                id: 'des_sett_<%=lingueTotali.get(i)%>',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_des_sett")%>(<%=lingueTotali.get(i)%>)",
                maxLength: 64000,
                allowBlank: false,
                name: 'des_sett_<%=lingueTotali.get(i)%>',
                labelStyle: 'font-weight:bold;'
...
<% } %>
            ...
            for (var ele=0;ele < dyn_fields_protection.length;ele++) {
                ...
                cod_sett=Ext.getUrlParam('cod_sett');
                if (cod_sett) {
                Ext.Ajax.request({
                url: 'leggiRecord',
                method: 'POST',
                params: {'cod_sett':cod_sett, 'table_name':table_name},
                success: function(response,request) {
                try {o = Ext.decode(response.responseText);}
                catch(e) {
                this.showError(response.responseText);
