...
var table_name = 'tipi_rif';
var sort_field = 'cod_tipo_rif';
...
{name: 'cod_tipo_rif'},
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
             {name: 'tipo_rif_<%=lingueTotali.get(i)%>'},
             <%}%>
{name: 'tipo_rif_it'}
];
var dyn_fields_protection = ['cod_tipo_rif'];
...
header: "<%=testiPortale.get(nomePagina + "_header_grid_cod_tipo_rif")%>",
width: 50,
sortable: true,
...
labelStyle: 'font-weight:bold;'
             ...
             <% for (int i = 0; i < lingueTotali.size(); i++){ %>
            {
                id: 'tipo_rif_<%=lingueTotali.get(i)%>',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_tipo_rif")%>(<%=lingueTotali.get(i)%>)",
                allowBlank: false,
                maxLength: 255,
                name: 'tipo_rif_<%=lingueTotali.get(i)%>',
                labelStyle: 'font-weight:bold;'
...
<% } %>            {
            ...
            for (var ele=0;ele < dyn_fields_protection.length;ele++) {
                ...
                cod_tipo_rif=Ext.getUrlParam('cod_tipo_rif');
                if (cod_tipo_rif) {
                Ext.Ajax.request({
                url: 'leggiRecord',
                method: 'POST',
                params: {'cod_tipo_rif':cod_tipo_rif, 'table_name':table_name},
                success: function(response,request) {
                try {o = Ext.decode(response.responseText);}
                catch(e) {
                this.showError(response.responseText);
