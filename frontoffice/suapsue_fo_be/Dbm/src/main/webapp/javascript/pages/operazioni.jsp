...
var table_name = 'operazioni';
var sort_field = 'cod_ope';
...
{name: 'cod_ope'},
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
             {name: 'des_ope_<%=lingueTotali.get(i)%>'},
             <%}%>
{name: 'des_ope_it'}
];

var dyn_fields_protection = ['cod_ope'];
...
labelStyle: 'font-weight:bold;'
            ...
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
            {
                id: 'des_ope_<%=lingueTotali.get(i)%>',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_des_ope")%>(<%=lingueTotali.get(i)%>)",
                maxLength: 64000,
                allowBlank: false,
                name: 'des_ope_<%=lingueTotali.get(i)%>',
                labelStyle: 'font-weight:bold;'
...
<% } %>
            ...
            for (var ele=0;ele < dyn_fields_protection.length;ele++) {
                ...
                cod_ope=Ext.getUrlParam('cod_ope');
                if (cod_ope) {
                Ext.Ajax.request({
                url: 'leggiRecord',
                method: 'POST',
                params: {'cod_ope':cod_ope, 'table_name':table_name},
                success: function(response,request) {
                try {o = Ext.decode(response.responseText);}
                catch(e) {
                this.showError(response.responseText);
