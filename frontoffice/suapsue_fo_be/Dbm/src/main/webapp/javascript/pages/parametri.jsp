...
var dyn_fields_protection = ['name','communeid'];
...
{
id:'grid_id',
header: "<%=testiPortale.get(nomePagina + "_header_grid_id")%>",
width: 20,
sortable: true,
dataIndex: 'id'
},
{
id:'grid_name',
header: "<%=testiPortale.get(nomePagina + "_header_grid_name")%>",
width: 50,
sortable: true,
dataIndex: 'name'
},
{
id:'grid_communeid',
header: "<%=testiPortale.get(nomePagina + "_header_grid_communeid")%>",
renderer: renderTpl,
width: 30,
sortable: true,
dataIndex: 'communeid'
...
{
id:'grid_value',
header: "<%=testiPortale.get(nomePagina + "_header_grid_value")%>",
renderer: renderTpl,
width: 100,
sortable: true,
dataIndex: 'value'
...
id: 'name',
fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_name")%>",
name: 'name',
allowBlank: false,
maxLength: 255,
...
var txtFldCommuneId = new Ext.form.TextField({
id: 'communeid',
fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_communeid")%>",
name: 'communeid',
maxLength: 255,
enableKeyEvents: true
});
var	itemsForm = [{
id: 'id',
fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_id")%>",
name: 'id',
readOnly: true
},txtFld,txtFldCommuneId,{
id: 'value',
fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_value")%>",
name: 'value',
maxLength: 400
},{
id: 'note',
...
for (var ele=0;ele < dyn_fields_protection.length;ele++) {
    ...
    Ext.MessageBox.confirm('Confirm', Ext.util.JSON.decode(request.response.responseText).failure.notifica, showResult);
    ...
    for (var ele=0;ele < dyn_fields_protection.length;ele++) {
        ...
        waitMsg: "<%=testiPortale.get("attesa_salvataggio")%>",
        success: function(result,request) {
        Ext.wego.msg('Status', Ext.util.JSON.decode(request.response.responseText).success.msg);
        ds.load({params: {start:0, size:grid_row}});
        Ext.getCmp('id').setValue(Ext.util.JSON.decode(request.response.responseText).success.id);
        Ext.getCmp('save').enable();
        Ext.getCmp('insert').disable();
        Ext.getCmp('cancel').enable();
        Ext.getCmp("azione_form").setValue('modifica');
