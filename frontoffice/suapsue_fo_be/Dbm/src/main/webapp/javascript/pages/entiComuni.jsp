...
%>
    ...
    Ext.onReady(function(){
        ...
        var table_name = 'enti_comuni';
        var sort_field = 'cod_ente';
        ...
        var dyn_fields = [
            ...
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
             {name: 'des_ente_<%=lingueTotali.get(i)%>'},
             <%}%>            
            ...
            {name: 'cod_bf'},
            {name: 'src_pyth'},
            {name: 'estensione_firma'},
            {name: 'cod_nodo'}
        ];

        //ClassiEnti
        var triggerClassiEnti = new Ext.ux.form.SearchTripleFieldForm({
            id: 'cod_classe_ente',
            fieldLabel:"<%=testiPortale.get(nomePagina + "_field_form_cod_classe_ente")%>",
            name: 'cod_classe_ente',
            window: winClassiEnti,
            hideTrigger1:false,
            clearButton: true,
            lookupFields:['cod_classe_ente','des_classe_ente'],
            windowLink: "<%=basePath%>protected/ClassiEnti.jsp?set_id=<%=set_id%>",
            windowLinkKeys:['cod_classe_ente'],
            formName: formName,
            url: urlScriviSessione,
            allowBlank: false,
            labelStyle: 'font-weight:bold;'
        });
        var dyn_fields_protection = ['cod_ente'];
        ...
        var colModel = new Ext.grid.ColumnModel([
            ...
            {
                id:'grid_cod_nodo',
                header: "<%=testiPortale.get(nomePagina + "_header_grid_cod_nodo")%>",
                renderer: renderTpl,
                width: 350,
                sortable: true,
                dataIndex: 'cod_nodo'
        ...
        var	itemsForm = [txtFld,{
            ...
            <% for (int i = 0; i < lingueTotali.size(); i++){ %>
            {
                id: 'des_ente_<%=lingueTotali.get(i)%>',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_des_ente")%>(<%=lingueTotali.get(i)%>)",
                xtype:'textarea',
                maxLength: 255,
                height: 37,
                allowBlank: false,
            ...
            triggerClassiEnti,
            {
                ...
                id: 'cod_istat',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_cod_istat")%>",
                maxLength: 20,
                name: 'cod_istat'
            },{
                id: 'cod_bf',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_cod_bf")%>",
                maxLength: 20,
                name: 'cod_bf'
            },{
                id: 'src_pyth',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_src_pyth")%>",
                name: 'src_pyth',
                xtype:'textarea',
                maxLength: 64000,
                height: 75
                ...
                id: 'estensione_firma',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_estensione_firma")%>",
                maxLength: 4,
                name: 'estensione_firma'
            },{
                id: 'cod_nodo',
                fieldLabel: "<%=testiPortale.get(nomePagina + "_field_form_cod_nodo")%>",
                maxLength: 45,
                name: 'cod_nodo'
            },{
                id: 'azione_form',
    ...
    <% }%>
            ...
            cod_ente=Ext.getUrlParam('cod_com');
            if (!cod_ente) {
                cod_ente=Ext.getUrlParam('cod_ente');
            }
            if (cod_ente) {
                Ext.Ajax.request({
                    url: 'leggiRecord',
                    method: 'POST',
                    params: {'cod_ente':cod_ente, 'table_name':table_name},
                    success: function(response,request) {
                        try {o = Ext.decode(response.responseText);}
                        catch(e) {
                            this.showError(response.responseText);
