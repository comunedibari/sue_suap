...
public class Documenti {
    ...
    private static Logger log = LoggerFactory.getLogger(Documenti.class);
    ...
    public JSONObject action(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if (utente.getRuolo().equals("A") || utente.getRuolo().equals("B")) {
                query = query + "where a.cod_doc in (select distinct cod_doc from allegati x "
                        + "join utenti_interventi y on x.cod_int=y.cod_int "
                        + "where y.cod_utente='" + utente.getCodUtente() + "') "
                        + "union "
                        + "select a.cod_doc, a.href, b.tit_doc, d.tit_href, e.nome_file, b.des_doc, b.cod_lang, a.tip_aggregazione "
                        + "from documenti a "
                        + "join documenti_testi b "
                        + "on a.cod_doc=b.cod_doc "
                        + "left join href c "
                        ...
                        + "on a.cod_doc=e.cod_doc "
                        + "and b.cod_lang=e.cod_lang "
                        + "where a.cod_doc not in "
                        + "(select cod_doc from allegati)";
            }
            query = query + ") p "
                    + "where p.cod_lang = 'it' and (p.tit_doc like ? or p.cod_doc like ? or p.href like ? or p.tit_href like ? or p.nome_file like ? or p.des_doc like ? )";
            ...
            if (!(Boolean) session.getAttribute("territorialitaNew")) {
                if (utente.getTipAggregazione() != null) {
                    st.setString(7, utente.getTipAggregazione());
                }
            }
            rs = st.executeQuery();
            if (rs.next()) {
            ...
            query = "select p.cod_doc cod_doc, p.flg_dic ,p.href href,p.tit_doc tit_doc,p.des_doc des_doc,p.tit_href tit_href,p.tip_doc tip_doc,p.nome_file nome_file, p.tip_aggregazione, p.des_aggregazione "
                    + "from (select a.cod_doc, a.href, b.tit_doc, d.tit_href, e.nome_file, b.des_doc, b.cod_lang, a.flg_dic, e.tip_doc, a.tip_aggregazione, h.des_aggregazione "
                    + "from documenti a "
                    + "join documenti_testi b "
                    + "on a.cod_doc=b.cod_doc "
                    + "left join href c "
            ...
            if (utente.getRuolo().equals("A") || utente.getRuolo().equals("B")) {
                query = query + "where a.cod_doc in (select distinct cod_doc from allegati x "
                        ...
                        + "where y.cod_utente='" + utente.getCodUtente() + "') "
                        + "union "
                        + "select a.cod_doc, a.href, b.tit_doc, d.tit_href, e.nome_file, b.des_doc, b.cod_lang, a.flg_dic, e.tip_doc, a.tip_aggregazione, h.des_aggregazione "
                        + "from documenti a "
                        + "left join tipi_aggregazione h "
                        + "on a.tip_aggregazione = h.tip_aggregazione "                        
                        + "join documenti_testi b "
            ...
            String queryLang = "select tit_doc, des_doc, cod_lang from documenti_testi where cod_doc = ?";
            ...
            while (rs.next()) {
                ...
                while (rsLang.next()) {
                    loopObj.put("des_doc_" + rsLang.getString("cod_lang"), Utility.testoDaDb(rsLang.getString("des_doc")));
...
//                loopObj.put("nome_file", rs.getString("nome_file"));
                ...
                if (rs.getString("nome_file") != null && !rs.getString("nome_file").equals("")) {
                    loopObj.put("nome_file", "<a href=\"ScaricaFile?tipo=documento&codDoc=" + rs.getString("cod_doc") + "&tipDoc=" + rs.getString("tip_doc") + "\"target=\"_blank\" alt=\"" + nomeFile + "\">" + nomeFile + "</a>");
                }
    ...
    public DocumentoModel leggi(String codDoc, String tipDoc) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            String query = "select a.cod_doc,a.nome_file,a.tip_doc,a.doc_blob "
                    + "from documenti_documenti a "
                    + "where a.cod_doc=? "
                    + "and   a.cod_lang='it' "
                    + "and   a.tip_doc=?";
            st = conn.prepareStatement(query);
    ...
    public JSONObject inserisciDocumento(DocumentoModel nm, HttpServletRequest request) throws Exception {
        Connection conn = null;
        PreparedStatement st = null;
        ResultSet rs = null;
        JSONObject ret = null;
        try {
            ...
            Modifica verifica = verificaNotifica(nm.getCodDoc(), request, nm, "caricaFile", conn);
            ret = new JSONObject();
            if (!verifica.isNotifica()) {
                String query = "delete from documenti_documenti where cod_doc = ?";
                st = conn.prepareStatement(query);
                st.setString(1, nm.getCodDoc());
                st.executeUpdate();
                Utility.chiusuraJdbc(st);
                query = "insert into documenti_documenti (cod_doc,tip_doc,cod_lang,nome_file,doc_blob) values (?,?,'it',?,?)";
                st = conn.prepareStatement(query);
                st.setString(1, nm.getCodDoc());
                ...
                funzRequest.put("documenti", param);
    ...
    public JSONObject inserisci(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            ret = new JSONObject();
            String query = "insert into documenti (cod_doc,href,flg_dic,tip_aggregazione) values (?,?,?,?)";
            st = conn.prepareStatement(query);
            st.setString(1, request.getParameter(("cod_doc")));
            st.setString(2, (request.getParameter("href") != null && !request.getParameter("href").equals("") ? request.getParameter("href") : null));
            st.setString(3, request.getParameter("flg_dic_hidden"));
            st.setString(4, request.getParameter("tip_aggregazione"));
            st.executeUpdate();
            Utility.chiusuraJdbc(st);
            query = "insert into documenti_testi (cod_doc,tit_doc,des_doc, cod_lang) values (?,?,?,?)";

            Set<String> parmKey = request.getParameterMap().keySet();
            for (String key : parmKey) {
                String[] parSplit = key.split("tit_doc_");
                if (parSplit.length > 1 && key.contains("tit_doc_")) {
                    String cod_lang = parSplit[1];
                    if (cod_lang != null) {
                        st = conn.prepareStatement(query);
                        st.setString(1, request.getParameter("cod_doc"));
                        st.setString(2, Utility.testoADb(request.getParameter("tit_doc_" + cod_lang)));
                        st.setString(3, (request.getParameter("des_doc_" + cod_lang) != null && !request.getParameter("des_doc_" + cod_lang).equals("") ? request.getParameter("des_doc_" + cod_lang) : null));
    ...
    public JSONObject aggiorna(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            Modifica verifica = verificaNotifica(request.getParameter("cod_doc"), request, null, "modificaDati", conn);
            ret = new JSONObject();
            if (!verifica.isNotifica()) {
                String query = "update documenti set flg_dic = ? ,href = ? where cod_doc=?";
                st = conn.prepareStatement(query);
                st.setString(1, request.getParameter(("flg_dic_hidden")));
                st.setString(2, (request.getParameter("href") != null && !request.getParameter("href").equals("") ? request.getParameter("href") : null));
                st.setString(3, request.getParameter(("cod_doc")));
                st.executeUpdate();
                Utility.chiusuraJdbc(st);
                if ((Boolean) session.getAttribute("territorialitaNew")) {
//                    if (utente.getTipAggregazione() != null) {
                        if (request.getParameter("tip_aggregazione") != null) {
                            query = "update documenti "
                                    ...
                                    + "where cod_doc =? and tip_aggregazione is null";
                            st = conn.prepareStatement(query);
...
//                    }
                ...
                String querySelect = "select * from documenti_testi where cod_doc = ? and cod_lang = ?";
                String queryInsert = "insert into documenti_testi (cod_doc,cod_lang,tit_doc,des_doc) values (?,?,?,? )";

                query = "update documenti_testi set tit_doc=? , des_doc = ? where cod_doc=? and cod_lang=?";
                ...
                for (String key : parmKey) {
                    ...
                    if (parSplit.length > 1 && key.contains("tit_doc_")) {
                        ...
                        if (cod_lang != null) {
                            ...
                            stRead.setString(1, request.getParameter("cod_doc"));
                            stRead.setString(2, cod_lang);
                            rsRead = stRead.executeQuery();
                            if (rsRead.next()) {
                                st = conn.prepareStatement(query);
                                st.setString(1, Utility.testoADb(request.getParameter(key)));
                                st.setString(2, (request.getParameter("des_doc_" + cod_lang) != null && !request.getParameter("des_doc_" + cod_lang).equals("") ? request.getParameter("des_doc_" + cod_lang) : null));
                                st.setString(3, request.getParameter("cod_doc"));
                                st.setString(4, cod_lang);
                                st.executeUpdate();
                                Utility.chiusuraJdbc(rs);
                                ...
                                st.setString(4, (request.getParameter("des_doc_" + cod_lang) != null && !request.getParameter("des_doc_" + cod_lang).equals("") ? request.getParameter("des_doc_" + cod_lang) : null));
                                st.execute();
                                Utility.chiusuraJdbc(rs);
                                Utility.chiusuraJdbc(st);
                            }
                ...
                scriviTCR(verifica, request.getParameter("cod_doc"), conn);
    ...
    private boolean cancellaControllo(String key, Connection conn) throws Exception {
        ...
        int righe = 0;
        String query = "select count(*) righe from allegati where cod_doc = ? "
                + "union "
                + "select count(*) righe from allegati_comuni where cod_doc=?";
        try {
            st = conn.prepareStatement(query);
            st.setString(1, key);
            st.setString(2, key);
    ...
    public JSONObject cancella(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            ret = new JSONObject();
            controllo = cancellaControllo(request.getParameter("cod_doc"), conn);
            if (controllo) {
                String query = "delete from documenti_documenti where cod_doc = ?";
                st = conn.prepareStatement(query);
                st.setString(1, request.getParameter("cod_doc"));
                st.executeUpdate();
                Utility.chiusuraJdbc(st);
                query = "delete from documenti_testi where cod_doc=?";
                st = conn.prepareStatement(query);
                st.setString(1, request.getParameter("cod_doc"));
                st.executeUpdate();
                Utility.chiusuraJdbc(st);
                query = "delete from documenti where cod_doc=?";
                st = conn.prepareStatement(query);
                st.setString(1, request.getParameter("cod_doc"));
                st.executeUpdate();
                ret.put("success", "Cancellazione effettuata");
            } else {
                ret.put("failure", "Documento in uso");
            }
        } catch (SQLException e) {
    ...
    public JSONObject cancellaDocumento(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            Modifica verifica = verificaNotifica(request.getParameter("cod_doc"), request, null, "cancellaDocumento", conn);
            ret = new JSONObject();
            if (!verifica.isNotifica()) {
                String query = "delete from documenti_documenti where cod_doc = ?";
                st = conn.prepareStatement(query);
    ...
    public Modifica verificaNotifica(String codRif, HttpServletRequest request, DocumentoModel nm, String azione, Connection conn) throws Exception {
        PreparedStatement st = null;
        ResultSet rs = null;
        HttpSession session = request.getSession();
        Utente utente = (Utente) session.getAttribute("utente");
        ...
        try {
            if (azione.equals("modificaDati")) {
                sql = "select a.cod_doc,b.des_doc,a.flg_dic,b.tit_doc,a.href "
                        + "from documenti a "
                        + "join documenti_testi b "
                        + "on a.cod_doc=b.cod_doc "
                        + "where a.cod_doc=? "
                ...
                rs = st.executeQuery();
                if (rs.next()) {
                    if (!request.getParameter("tit_doc_it").equals(rs.getString("tit_doc"))
                            || !request.getParameter("des_doc_it").equals(rs.getString("des_doc"))
                            || !request.getParameter("href").equals(rs.getString("href"))
                            || !request.getParameter("flg_dic_hidden").equals(rs.getString("flg_dic"))) {
                ...
                sql = "select a.cod_doc,a.nome_file,a.tip_doc,length(a.doc_blob) len "
                        + "from documenti_documenti a "
                        + "where a.cod_doc=? "
                        + "and a.cod_lang='it'";
                st = conn.prepareStatement(sql);
            ...
            if (utente.getRuolo().equals("A") || utente.getRuolo().equals("B")) {
                if (modificato.isModificaIntervento()) {
                    sql = "select count(*) righe from "
                            + "(select distinct cod_int from allegati "
                            + "where cod_doc = ? ) b "
    ...
    public JSONObject scriviNotifica(HttpServletRequest request) throws Exception {
        ...
        if (funzRequest.containsKey("documenti")) {
            param = (HashMap<String, Object>) funzRequest.get("documenti");
        } else {
        ...
        List<String> li = null;
        DocumentoModel nm = null;
        try {
            conn = Utility.getConnection();
            ret = new JSONObject();
            String codDoc = null;
            if (request.getParameter("table_name").equals("documenti_documenti_upload")) {
                nm = (DocumentoModel) param.get("fileObject");
                codDoc = (String) nm.getCodDoc();
            } else {
                codDoc = (String) param.get("cod_doc");
            }
            ...
            String sqlDeferred = "";
            if (listaUtentiInterventi.size() > 0) {
                if (request.getParameter("table_name").equals("documenti_documenti")) {
                    msg = "Id Documento = " + codDoc + "\n";
                    msg = msg + "Nuovo titolo documento = " + param.get("tit_doc_it") + "\n";
                    msg = msg + "Richiesta cancellazione documento fisico \n";
                    msg = msg + request.getParameter("textMsg");
                } else if (request.getParameter("table_name").equals("documenti_documenti_upload")) {
                    msg = "Id Documento = " + nm.getCodDoc() + "\n";
                    ...
                    msg = "Id Documento = " + codDoc + "\n";
                    msg = msg + "Nuovo titolo documento = " + param.get("tit_doc_it") + "\n";
                    msg = msg + "Nuovo descrizione documento = " + param.get("des_doc_it") + "\n";
                    msg = msg + "Nuovo codice dichiarazione = " + param.get("href") + "\n";
                    msg = msg + "Nuovo indicatore dichiarazione = " + param.get("flg_dic_hidden") + "\n";
                    msg = msg + request.getParameter("textMsg");
                }
                ...
                st.setString(2, msg);
                st.setString(3, "N");
                st.setString(4, "Documenti=" + codDoc);
                ...
                if (request.getParameter("table_name").equals("documenti_documenti")) {
                    // componi stringa sql
                    sqlDeferred = "delete from documenti_documenti where cod_doc = '" + codDoc + "'";
                    ...
                    Utility.chiusuraJdbc(st);
                    // componi stringa sql
                    sqlDeferred = "delete from documenti_documenti where cod_doc = '" + nm.getCodDoc() + "'";
                    // carica stringa sql nell'xml
                    doc.getSqlString().add(sqlDeferred.getBytes());

                    sqlDeferred = "insert into documenti_documenti (cod_doc,tip_doc,cod_lang,nome_file,doc_blob) select '" + nm.getCodDoc() + "',"
                            + "tip_doc,cod_lang,nome_file,doc_blob from notifiche_documenti where cnt = " + contatore;
                    doc.getSqlString().add(sqlDeferred.getBytes());

                } else {
                    // componi stringa sql
                    sqlDeferred = "update documenti_testi set tit_doc = '" + Utility.testoADbNotifica((String) param.get("tit_doc_it")) + "', "
                            + "des_doc = '" + Utility.testoADbNotifica((String) param.get("des_doc_it")) + "' "
                            + "where cod_doc = '" + codDoc + "' and cod_lang='it';";
                    // carica stringa sql nell'xml
                    doc.getSqlString().add(sqlDeferred.getBytes());
                    sqlDeferred = "update documenti set href = '" + Utility.testoADbNotifica((String) param.get("href")) + "', "
                            + "flg_dic = '" + param.get("flg_dic_hidden") + "' where cod_doc = '" + codDoc + "'";
                    doc.getSqlString().add(sqlDeferred.getBytes());
                }
                sqlDeferred = "update interventi set data_modifica=current_timestamp where cod_int in (select distinct cod_int from allegati where cod_doc = '" + codDoc + "')";
                doc.getSqlString().add(sqlDeferred.getBytes());
    ...
    public void scriviTCR(Modifica modificato, String codDoc, Connection conn) throws Exception {
        PreparedStatement st = null;
        ResultSet rs = null;
        String sql = null;
        try {
            if (modificato.isModificaIntervento()) {
                sql = "update interventi set data_modifica=current_timestamp where cod_int in (select distinct cod_int from allegati where cod_doc = ?)";
                st = conn.prepareStatement(sql);
                st.setString(1, codDoc);
                st.executeUpdate();
                Boolean aggiorna = Boolean.parseBoolean(Configuration.getConfigurationParameter("updateTCR").toLowerCase());
                if (aggiorna) {
                    sql = "update interventi set flg_pubblicazione= 'S' where cod_int in (select distinct cod_int from allegati where cod_doc = ?)";
                    st = conn.prepareStatement(sql);
                    st.setString(1, codDoc);
                    st.executeUpdate();
                }
