 ...
 * 20/set/2011 22.59.21
 */
public class ProtocolloSegnatura {

    private static Logger log = LoggerFactory.getLogger(ProtocolloSegnatura.class);
    ...
    public JSONObject action(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            ret = new JSONObject();
            ret.put("dati_protocollo_segnatura", riga);
        } finally {
    ...
    public JSONObject aggiorna(HttpServletRequest request) throws Exception {
    	...
    	if (!riga.isEmpty()) {
	        ...
	        try {
	            ...
	            String query = "delete from sportelli_param_prot_pec where ref_sport = ?";
	            st = conn.prepareStatement(query);
	            st.setString(1, ref_sport);
	            st.executeUpdate();
	            st.close();
	            
	            query = "insert into sportelli_param_prot_pec(ref_sport, name, value) values(?, ?, ?)";
