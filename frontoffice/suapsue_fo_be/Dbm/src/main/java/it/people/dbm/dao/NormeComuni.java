 ...
 * @author Piergiorgio
 */
public class NormeComuni {
    private static Logger log = LoggerFactory.getLogger(NormeComuni.class);
    ...
    public JSONObject action(HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            query = "select count(*) righe "
                    ...
                    + "join normative_testi c "
                    + "on a.cod_rif = c.cod_rif "
                    + "join enti_comuni d "
                    ...
                    + "where c.cod_lang='it' "
                    + "and (a.cod_rif like ? or a.cod_com like ? or c.tit_rif like ? or e.des_ente like ? ) ";
            ...
            query = "select a.cod_rif cod_rif,c.tit_rif tit_rif,a.cod_com cod_com,e.des_ente des_ente, a.flg flg "
                    + "from norme_comuni a "
                    + "join normative b "
                    + "on a.cod_rif=b.cod_rif "
                    + "join normative_testi c "
    ...
    public JSONObject inserisci(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            ret = new JSONObject();
            query = "insert into norme_comuni (cod_rif,cod_com,flg) "
                    + "values (?,?,?)";
            st = conn.prepareStatement(query);
    ...
    public JSONObject aggiorna(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            ret = new JSONObject();
            query = "update norme_comuni set flg=? "
                    + "where cod_rif=? and cod_com=?";
            st = conn.prepareStatement(query);
    ...
    public JSONObject cancella(HttpServletRequest request) throws Exception {
        ...
        try {
            conn = Utility.getConnection();
            ret = new JSONObject();
            query = "delete from norme_comuni where cod_rif=? and cod_com=?";
            st = conn.prepareStatement(query);
            st.setString(1, request.getParameter("cod_rif"));
            st.setString(2, request.getParameter("cod_com"));
            st.executeUpdate();
