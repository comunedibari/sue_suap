 ...
 */
public class OracleLazyPagedListHolder extends AbstractLazyPagedListHolder {

	protected static final String BACKED_COUNT_QUERY = "select count(*) as \"" + COUNT_COLUMN_NAME + "\" from ( select /*+ FIRST_ROWS(n) */ a.*, ROWNUM rnum from ($" + QUERY_PLACE_HOLDER + "$) a)";
	protected static final String BACKED_QUERY = "select * from ( select /*+ FIRST_ROWS(n) */ a.*, ROWNUM rnum from ($" + QUERY_PLACE_HOLDER + "$) a where ROWNUM <= $" + UPPER_PAGE_LIMIT + "$ ) where rnum  >= $" + LOWER_PAGE_LIMIT + "$";

	public OracleLazyPagedListHolder(final String pagedListId, final DataSource dataSource, 
	...
	public void nextPage() {
		...
		if (!this.isLastPage()) {
			...
			this.setUpperPageLimit(this.getUpperPageLimit() + this.getPageSize());
	...
	public void setPage(int pageNumber) {
		...
		if (pageNumber <= this.getPageCount() && pageNumber > 0) {
			...
			this.setLowerPageLimit(1 + ((pageNumber - 1) * this.getPageSize()));
			this.setUpperPageLimit(this.getLowerPageLimit() + this.getPageSize() - 1);
			this.setPageList(boundPageList(this.getBackedQuery(), 
					this.getLowerPageLimit(), this.getUpperPageLimit()));
		}
