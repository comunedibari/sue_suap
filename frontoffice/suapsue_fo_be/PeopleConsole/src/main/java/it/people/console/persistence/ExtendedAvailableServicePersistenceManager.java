...
import it.people.console.domain.ExtendedAvailableService;
 ...
 *
 */
public class ExtendedAvailableServicePersistenceManager extends AbstractPersistenceManager {

    protected Class<?> typeManaged = ExtendedAvailableService.class;
	
	public ExtendedAvailableServicePersistenceManager() {
		super(Mode.READ);
	}

	public ExtendedAvailableServicePersistenceManager(Mode mode) {
		super(mode);
	}
	...
	protected Collection<?> _get(Class<?> clazz)
		...
		return persistenceBroker.getCollectionByQuery(query);
	...
	protected Collection<?> _get(Query query)
		...
		if (query instanceof QueryByIdentity) {
			...
			Object obj = persistenceBroker.getObjectByQuery(query);
			if (obj != null) {
				results.add(obj);
			}
			return results;
	...
	protected void _set(Object object) throws PersistenceManagerException {
        ...
        if (this.typeManaged.isAssignableFrom(object.getClass())) {
            try {
            	persistenceBroker.store((ExtendedAvailableService) object);
            } catch (Exception ex) {
                logger.error("", ex);
                throw new PersistenceManagerDBAccessException();
            }
        } else {
            throw new IllegalArgumentException("FEService expected!");
        }
	 ...
	 */
	public ExtendedAvailableService get(Object oid) throws PersistenceManagerException {
		Identity identity = persistenceBroker.serviceIdentity().buildIdentity(ExtendedAvailableService.class, oid);
		return (ExtendedAvailableService)persistenceBroker.getObjectByIdentity(identity);
	}
