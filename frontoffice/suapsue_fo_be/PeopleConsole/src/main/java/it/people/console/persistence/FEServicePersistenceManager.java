 ...
 *
 */
public class FEServicePersistenceManager extends AbstractPersistenceManager {

    protected Class<?> typeManaged = FEService.class;
	
	public FEServicePersistenceManager() {
		super(Mode.READ);
	}

	public FEServicePersistenceManager(Mode mode) {
		super(mode);
	}
	...
	protected void _set(Object object) throws PersistenceManagerException {
        ...
        if (this.typeManaged.isAssignableFrom(object.getClass())) {
            try {
            	getPersistenceBroker().store((FEService) object);
            } catch (Exception ex) {
                logger.error("", ex);
	 ...
	 */
	public FEService get(Object oid) throws PersistenceManagerException {
		Identity identity = getPersistenceBroker().serviceIdentity().buildIdentity(FEService.class, oid);
		return (FEService)getPersistenceBroker().getObjectByIdentity(identity);
	}
