...
public class SubmittedProcessPersistenceMgr extends PersistenceManager {

    private Category cat = Category
	    .getInstance(SubmittedProcessPersistenceMgr.class.getName());

    public SubmittedProcessPersistenceMgr(PersistenceManager.Mode p_obj_mode) {
	super(p_obj_mode);
    }
	...
	if (p_obj_process instanceof SubmittedProcess) {
	    try {
	    ...
	    throw new IllegalArgumentException("SubmittedProcess expected!");
	}
    }
	...
	if (p_obj_Object instanceof SubmittedProcess) {
	    SubmittedProcess p_obj_process = (SubmittedProcess) p_obj_Object;

	    ClassDescriptor cd = m_obj_ojbBroker
		    .getClassDescriptor(SubmittedProcess.class);
	    if (cd == null) {
		throw new IllegalArgumentException(
			"Unable to find class descriptor for class SubmittedProcess");
	    }
