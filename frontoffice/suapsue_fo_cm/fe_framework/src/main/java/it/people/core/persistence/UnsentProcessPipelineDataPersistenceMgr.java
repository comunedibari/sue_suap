...
public class UnsentProcessPipelineDataPersistenceMgr extends PersistenceManager {

    private Category cat = Category
	    .getInstance(UnsentProcessPipelineDataPersistenceMgr.class
		    .getName());

    public UnsentProcessPipelineDataPersistenceMgr(
	...
	if (p_obj_process instanceof UnsentProcessPipelineData) {
	    try {
	    ...
	    throw new IllegalArgumentException(
		    "UnsentProcessPipelineData expected!");
	}
    }
	...
	if (p_obj_Object instanceof UnsentProcessPipelineData) {
	    UnsentProcessPipelineData p_obj_process = (UnsentProcessPipelineData) p_obj_Object;

	    ClassDescriptor cd = m_obj_ojbBroker
		    .getClassDescriptor(UnsentProcessPipelineData.class);
	    if (cd == null) {
		throw new IllegalArgumentException(
			"Unable to find class descriptor for class UnsentProcessPipelineData");
	    }
		...
		if (newOID == null) {
		    throw new NullPointerException(
			    "Error retrieving UnsentProcessPipelineData OID from Sequence");
		}
