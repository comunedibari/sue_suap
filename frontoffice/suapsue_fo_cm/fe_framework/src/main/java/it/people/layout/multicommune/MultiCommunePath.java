...
public class MultiCommunePath {
     ...
     */
    public MultiCommunePath(HttpServletRequest request, String rootPath) {
	this.session = request.getSession();
	this.rootPath = rootPath;
    }

    public MultiCommunePath(HttpSession session, String rootPath) {
     ...
     * 'tomcat_root'/'people'/'root_path'/'codice_comune'/'nestedPath'/'elementName
     * ' l'elemento di default � dato dal path:
     * 'tomcat_root'/'people'/'root_path'/default/'nestedPath'/'elementName'
     * 
     * @param nestedPath
     ...
     */
    public String getPath(String nestedPath, String elementName) {
	...
	String key = this.rootPath + "/" + communeId + "/" + elementName;
	...
	if (path == null) {
	    ...
	    path = getPath(communeId, nestedPath, elementName);

	    // verifico se esiste il file specializzato per comune
	    if (!fileExist(path))
		path = getPath("default", nestedPath, elementName);
    ...
    protected String getPath(String whereToSearch, String nestedPath,
	    String elementName) {
	String path = rootPath + "/" + whereToSearch + nestedPath + "/"
		+ elementName;
	return path;
    }
