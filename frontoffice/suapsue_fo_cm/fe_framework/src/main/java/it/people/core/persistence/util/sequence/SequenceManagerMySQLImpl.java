...
//DeCompiled : SequenceManagerMySQLImpl.class
package it.people.core.persistence.util.sequence;
...
public class SequenceManagerMySQLImpl implements SequenceManager {
    ...
    public void afterStore(JdbcAccess jdbcaccess,
	...
	if (fielddescriptor != null) {
	    ...
	    try {
		resultsetandstatement = jdbcaccess.executeSQL(
			"SELECT LAST_INSERT_ID() as newid FROM "
				+ classdescriptor.getFullTableName(),
			classdescriptor, Query.NOT_SCROLLABLE);
		int i = 0;
		if (resultsetandstatement.m_rs.next())
		    i = resultsetandstatement.m_rs.getInt("newid");
		resultsetandstatement.m_rs.close();
		if (log.isDebugEnabled())
		    log.debug("After store - newid=" + i);
		PersistentField persistentfield = fielddescriptor
			.getPersistentField();
		FieldConversion fc = fielddescriptor.getFieldConversion();
		if (persistentfield.getType().getName()
	...
	for (Iterator iterator = vector.iterator(); iterator.hasNext();) {
	    ObjectReferenceDescriptor objectreferencedescriptor = (ObjectReferenceDescriptor) iterator
		    .next();
	    Object obj1 = objectreferencedescriptor.getPersistentField().get(
		    obj);
	    if (obj1 != null)
		assertFkAssignment(obj, classdescriptor, obj1,
			objectreferencedescriptor);
	}

    }

    private void assertFkAssignment(Object obj,
	    ClassDescriptor classdescriptor, Object obj1,
	    ObjectReferenceDescriptor objectreferencedescriptor) {
	Class class1 = ProxyHelper.getRealClass(obj1);
	ClassDescriptor classdescriptor1 = broker.getDescriptorRepository()
		.getDescriptorFor(class1);
	ClassDescriptor classdescriptor2 = classdescriptor;
	FieldDescriptor afielddescriptor[] = objectreferencedescriptor
		.getForeignKeyFieldDescriptors(classdescriptor2);
	if (afielddescriptor != null) {
	    FieldDescriptor fielddescriptor = classdescriptor1
		    .getAutoIncrementFields()[0];
	    Object obj2 = null;
	    for (int i = 0; i < afielddescriptor.length; i++) {
		FieldDescriptor fielddescriptor1 = afielddescriptor[i];
		fielddescriptor1.getPersistentField().set(obj,
			fielddescriptor.getPersistentField().get(obj1));
	    }
    ...
    /**
     * Returns a unique object for the given field attribute. The returned value
     * takes in account the jdbc-type and the FieldConversion.sql2java()
     * conversion defined for <code>field</code>. The returned object is unique
     * accross all tables in the extent of class the field belongs to.
     */
