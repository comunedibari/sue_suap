...
public class UserSignalledBugsPersistenceMgr extends PersistenceManager {

    private Category cat = Category
	    .getInstance(UserSignalledBugsPersistenceMgr.class.getName());

    public UserSignalledBugsPersistenceMgr(PersistenceManager.Mode p_obj_mode) {
	super(p_obj_mode);
    }
	...
	if (p_obj_process instanceof UserSignalledBug) {
	    try {
	    ...
	    throw new IllegalArgumentException("UserSignalledBug expected!");
	}
    }
	...
	if (p_obj_Object instanceof UserSignalledBug) {
	    UserSignalledBug p_obj_process = (UserSignalledBug) p_obj_Object;

	    ClassDescriptor cd = m_obj_ojbBroker
		    .getClassDescriptor(UserSignalledBug.class);
	    if (cd == null) {
		throw new IllegalArgumentException(
			"Unable to find class descriptor for class UserSignalledBug");
	    }
		...
		if (newOID == null) {
		    throw new NullPointerException(
			    "Error retrieving UserSignalledBug OID from Sequence");
		}
