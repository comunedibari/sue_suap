...
public class NotSubmittableProcessPersistenceMgr extends PersistenceManager {

    private Category cat = Category
	    .getInstance(NotSubmittableProcessPersistenceMgr.class.getName());

    public NotSubmittableProcessPersistenceMgr(
	...
	if (p_obj_process instanceof NotSubmittableProcess) {
	    try {
	    ...
	    throw new IllegalArgumentException(
		    "NotSubmittableProcess expected!");
	}
    }
	...
	if (p_obj_Object instanceof NotSubmittableProcess) {
	    NotSubmittableProcess p_obj_process = (NotSubmittableProcess) p_obj_Object;

	    ClassDescriptor cd = m_obj_ojbBroker
		    .getClassDescriptor(NotSubmittableProcess.class);
	    if (cd == null) {
		throw new IllegalArgumentException(
			"Unable to find class descriptor for class NotSubmittableProcess");
	    }
