 ...
 */
public class InitProcess extends Action {
	...
	try {
	    ...
	    if (serviceConf == null)
		throw new Exception("Il servizio � inesistente.");
		...
		if (loadPplProcess((AbstractPplProcess) p_actionForm,
		    ...
		    ((AbstractPplProcess) p_actionForm).getData()
			    .onLoad((AbstractPplProcess) p_actionForm,
				    p_servletRequest);
	    ...
	    cat.info(ppex);
	    return p_actionMapping.findForward("pendingPayment");
	} catch (Exception ex) {
	    cat.error(ex);
	}
	return p_actionMapping.findForward("failed");
	...
	try {
	    ...
	    // IProcess process = wFlow.getProcess(processType, curCity);
	    IProcess process = wFlow.getProcess(p_servletRequest.getSession(),
		    processType, curCity);
	...
	try {
		...
		this.error = MessagesFactory.getInstance().getErrorMessage(
			pplContext.getCommune().getKey(), "pagamento.abortito");
		this.error.setErrorForward(ERROR_JSP);
		p_servletRequest.setAttribute("errorMessage", this.error);

	    } else if (processStatusTable.getStatus() == ProcessStatus.PAYMENT_OK
		    .getStatusCode()) {
